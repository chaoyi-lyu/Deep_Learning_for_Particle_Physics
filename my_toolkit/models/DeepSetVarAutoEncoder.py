...
class LossFunc(nn.Module):
    ...
    def forward(self, kine_input, class_input, kine_pred, class_pred, mu, log_var):
        ...
        total_loss = (1 - self.beta) * (Chamfer_loss + self.w * Class_loss + self.c * ClassNum_loss) + self.beta * KL_loss
        return total_loss
    ...
    def ClassNumLoss(self, class_input, class_pred):
        ...
        for i in range(class_input.shape[0]):
            input_num = torch.bincount(torch.max(class_input, axis=2)[-1][i], minlength=9)
            pred_num = torch.bincount(torch.max(class_pred.exp(), axis=2)[-1][i], minlength=9)
